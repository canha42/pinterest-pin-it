...
function pibfi_Engine_configs() {
	if (isset($_POST['submit'])) {
		...
		?><div class="updated"><p><strong><?php echo __('Options saved.', 'ppibfi_translate'); ?></strong></p></div><?php
	}
...
function checkedAll () {var aa= document.getElementById('pinpages');checked = !checked;for (var i =0; i < aa.elements.length; i++) {aa.elements[i].checked = checked;}}</script>
	<div class="wrap">
		...
		<div class="xcpinc">
		<p><?php echo __('Here you may change some settings for your Pinterest PIBFI plugin, although it is not necessary (we already have the optimal settings done for you).', 'ppibfi_translate'); ?></p>
		<form method="post" action="#" id="frm1">
		
		<fieldset>
			<legend><?php echo __('Content width:','ppibfi_translate'); ?>	</legend>
			<p><?php echo __('The plugin needs to know the width of the content area in pixels to place the button (only numbers).','ppibfi_translate'); ?> </p>
			<p>
		...
		<fieldset id="pinpages">
			<legend><?php echo __('Show "Pin It" button on following pages:','ppibfi_translate'); ?> </legend>
			<p>
			<input type="checkbox" name="ppibfi_pg_all" id="ppibfi_pg_all" onclick="checkedAll();" />
			<label for="ppibfi_pg_all"><?php echo __('All pages','ppibfi_translate'); ?> </label>
			</p>
			<p>
			<input type="checkbox" name="ppibfi_pg_index" id="ppibfi_pg_index" <?php echo $xcp_index?> />
			<label for="ppibfi_pg_index"><?php echo __('Index / home','ppibfi_translate'); ?> </label>
			</p>
			<p>
			<input type="checkbox" name="ppibfi_pg_single" id="ppibfi_pg_single" <?php echo $xcp_single?> />
			<label for="ppibfi_pg_single"><?php echo __('Single post','ppibfi_translate'); ?> </label>
			</p>
			<p>
			<input type="checkbox" name="ppibfi_pg_cat" id="ppibfi_pg_cat" <?php echo $xcp_cat?> />
			<label for="ppibfi_pg_cat"><?php echo __('Category page','ppibfi_translate'); ?>	</label>
			</p>
			<p>
			<input type="checkbox" name="ppibfi_pg_page" id="ppibfi_pg_page" <?php echo $xcp_page?> />
			<label for="ppibfi_pg_page"><?php echo __('Page','ppibfi_translate'); ?> </label>
			</p>
		</fieldset>
		
		<fieldset>
			<legend><?php echo __('Opt-out on single pages:','ppibfi_translate'); ?> </legend>
			<p><?php echo __('Option to enable a checkbox on singles and pages that will let you choose if the plugin will be deactivated on that particular page','ppibfi_translate'); ?> </p>
			<p>
			<input type="checkbox" name="ppibfi_opt_enable" id="ppibfi_opt_enable" value="on" <?php echo $xcp_opt_enable?> />
			<label for="ppibfi_opt_enable"><?php echo __('Enable opt-out','ppibfi_translate'); ?> </label>
			</p>
		</fieldset>
		
		<input type="submit" name="submit" value="<?php echo __('Save', 'ppibfi_translate'); ?>" class="xcp_submit" />
		
	</div><!-- xcpinc -->
	<div class="xcpf">
		<h3><?php echo __('Advanced','ppibfi_translate'); ?> </h3>
		<fieldset id="advanced">
			<legend><?php echo __('Exclude classes:','ppibfi_translate'); ?> </legend>
			<p><?php echo __('Images with the following "class" attribute won\'t display the button (comma separated):','ppibfi_translate'); ?> </p>
			<input type="text" name="ppibfi_exclude" id="ppibfi_exclude" value="<?php echo implode(',', $ppibfi_exclude); ?>" />
		</fieldset>
		
		<fieldset>
			<legend><?php echo __('Selected images only:','ppibfi_translate'); ?> </legend>
			<p>
			<input type="checkbox" name="ppibfi_img_pinthis" id="ppibfi_img_pinthis" <?php echo $xcp_img?> />
			<label for="ppibfi_img_pinthis"><?php echo __('Show "Pin it" button only on images with "pinthis" class','ppibfi_translate'); ?> </label>
			</p>
			<p><?php echo __('This option will overide the "Exclude classes" and "Enable opt-out" options.','ppibfi_translate'); ?> </p>
		</fieldset>
	...
	<div class="xcpf">
		<h3><?php echo __('Care to help?','ppibfi_translate'); ?> </h3>
		<p><?php echo __('This plugin takes up a *lot* of my spare time and has cost me around US$ 500 in development out of my own pocket. Any donation amount (like five bucks) will be well appreciated as it\'ll give me more reason to work hard on new updates. Please consider donating! Link to PayPal below.','ppibfi_translate'); ?> </p>
		<p><?php echo __('If you are a programmer willing to help out with the development, please drop me a line at canha (at) design.blog.br. But if you don\'t have any "mad skillz", you can also help by reporting bugs at <a href="https://github.com/canha42/pinterest-pin-it/issues">GitHub</a>. Please help spread the word about this plugin!','ppibfi_translate'); ?> </p>
		<p><?php echo __('Thanks!','ppibfi_translate'); ?> </p>
